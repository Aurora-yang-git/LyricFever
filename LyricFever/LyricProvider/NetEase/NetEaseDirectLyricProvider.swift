//
//  NetEaseDirectLyricProvider.swift
//  Lyric Fever
//
// Drop-in replacement for NetEaseLyricProvider.
// When MUSIC_U + __csrf are saved in UserDefaults, calls music.163.com/api/ directly
// so mainland China users don't need the Vercel proxy.
// Falls back to the Vercel URL automatically when no cookies are configured.

import Foundation
import StringMetric

class NetEaseDirectLyricProvider: LyricProvider {
    ...
    // MARK: - Unified search/lyric helpers (pick endpoint based on cookie presence)
    ...
    func fetchNetworkLyrics(trackName: String, trackID: String, currentlyPlayingArtist: String?, currentAlbumName: String?) async throws -> NetworkFetchReturn {
        ...
        let cleaned = unescapeNetEaseHTMLEntities(in: lrcString)
    ...
    func search(trackName: String, artistName: String) async throws -> [SongResult] {
        ...
        for song in searchResult.result.songs {
            ...
            do {
                ...
                let cleaned = unescapeNetEaseHTMLEntities(in: lrcText)
...
private func unescapeNetEaseHTMLEntities(in text: String) -> String {
    var s = text
